    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <Link to="/kafans-shirts" className="inline-block text-yellow-400 hover:text-yellow-300 underline underline-offset-4">
            {t('product.backHome')}
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 px-4 md:px-10">
      ...
      <nav className="max-w-7xl mx-auto flex items-center gap-2 text-sm text-gray-400 mb-8">
        ...
        <Link to="/kafans-shirts" className="hover:text-yellow-400 transition-colors">Kaftan</Link>
        <FaChevronRight className="w-2.5 h-2.5" />
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 space-y-6 lg:pt-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => {
                const wishlisted = isInWishlist(product.id, 'Kaftan');
                if (wishlisted) {
                  removeFromWishlist(product.id, 'Kaftan');
                  toast.success(t('product.removedFromWishlist'));
                } else {
                  addToWishlist({ id: product.id, name: product.name, price: product.price, image: product.image, route: `/kafans-shirts/${product.id}`, category: 'Kaftan' });
              ...
              className={`w-12 h-12 flex items-center justify-center rounded-xl border transition-all duration-200 cursor-pointer ${
                isInWishlist(product.id, 'Kaftan')
                  ? 'border-red-500/50 bg-red-500/10 text-red-500'
                  : 'border-zinc-700 hover:border-zinc-500 text-gray-400 hover:text-red-400'
              }`}
            >
              {isInWishlist(product.id, 'Kaftan') ? <FaHeart size={18} /> : <FaRegHeart size={18} />}
            </button>
          </div>
        </div>
      </div>
